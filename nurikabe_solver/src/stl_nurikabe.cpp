...
string boardToString(const EGM::BaseBoard& iBoard) {
  stringstream os;

  for_each (iBoard.begin(), iBoard.end(), [&](EGM::int_type value) {
    switch (value) {
      case EGM::Cell::VOID: 
        os << " ,";
        break;
  ...
  LOG_DEBUG("Built string from board : " << endl << os.str());
  return os.str();
}

void solve_board(const EGM::BaseBoard& iBoard) {
  uint_fast32_t iteration_count = 0;
  Grid g(iBoard.xlen(), iBoard.ylen(), boardToString(iBoard));
  LOG_INFO("STL : trying to solve :" << iBoard.toString());

  STAT_HIGH_PRIO_START(STL::solve_board);
  while (g.solve(STL_VERBOSE, true) == Grid::KEEP_GOING) { ++iteration_count; }
  STAT_HIGH_PRIO_STOP(STL::solve_board);

  LOG_DEBUG("STL : Internal state : " << g.toString());
  LOG_INFO("STL : Result (iterations=" << iteration_count << ") : " << g.toBaseBoard().toString());
  ASSERT(g.known() == iBoard.cellCount(), "STL solver did not solve the board completely");
}
...
Grid::Grid(const int width, const int height, const string& s)
    ...
    for (boost::sregex_iterator i(s.begin(), s.end(), rx), end; i != end; ++i) {
        const boost::smatch& m = *i;
        ...
        if (m[1].matched) {
            ...
            throw runtime_error("RUNTIME ERROR: Grid::Grid() - "
                "s must contain only digits, spaces, commas/newlines (as delimiters).");
        }
    }
    ...
    for (int x = 0; x < width; ++x) {
        for (int y = 0; y < height; ++y) {
            ...
            if (n != 0) {
                ...
                if (valid(x, y - 1) && n > 0 && cell(x, y - 1) > 0) {
                    LOG_ERROR("Stl validate error (" << x << "," << y << ") : " << s);
                    throw runtime_error("RUNTIME ERROR: Grid::Grid() - s contains vertically adjacent numbers.");
                }
    ...
    m_total_black = m_width*m_height - accumulate(v.begin(), v.end(), 0, [](int acc, int value) {
      return acc + (value > 0 ? value : 0);
    });

    LOG_DEBUG("After construction : " << toString());
...
Grid::SitRep Grid::solve(const bool verbose, const bool guessing) {
    ...
    for (auto i = m_regions.begin(); i != m_regions.end(); ++i) {
        ...
        const bool partial = (r.black() && r.size() < m_total_black)
                || r.white()
...
void Grid::print(const string& s, const set<pair<int, int>>& updated) {
    LOG_WARN("STL : " << s << " : " << EGM::toStrHelper(updated));
...
EGM::int_type toCellValue (Grid::State stl_value) {
  using EGM::Cell;
  switch (stl_value) {
    case Grid::WHITE: return Cell::RED;
    case Grid::BLACK: return Cell::BLUE;
    case Grid::UNKNOWN: return Cell::VOID;
    default: return stl_value;
  }
}

EGM::BaseBoard Grid::toBaseBoard() const {
  EGM::BaseBoard board;
  for (uint_fast32_t i=0, x=0, y=0; i < m_width*m_height; ++i, x=i%m_width, y=i/m_width) {
    State stl_value = cell(x,y);
    board.set(x, y, toCellValue(stl_value));
  }
  return board;
}

string Grid::Region::toString() const {
  stringstream os;
  os << "{ state=" << m_state << ", coords=" << EGM::toStrHelper(m_coords) 
    << ", m_unknowns=" << EGM::toStrHelper(m_unknowns) << " }";
  return os.str();
}

string Grid::toString() const {
  stringstream os;
  os << "Grid (m_width=" << m_width << ", m_height=" << m_height << ", m_total_black=" << m_total_black
    << ", m_sitrep=" << m_sitrep <<")";
